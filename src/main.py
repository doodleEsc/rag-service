...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:  # noqa: ARG001
    ...
    if is_leader:
        ...
        active_resources = [
            r for r in resource_service.get_all_resources() if r.status == "active"
        ]
...
watched_resources: dict[
    str, BaseObserver
] = {}  # Directory path -> Observer instance mapping
...
class RetrieveRequest(BaseModel):
    ...
    top_k: int | None = Field(
        5, description="Number of top results to return", ge=1, le=20
    )
...
class FileSystemHandler(FileSystemEventHandler):
    ...
    def handle_file_change(self: FileSystemHandler, file_path: Path) -> None:
        ...
        if (
            ...
            and current_time - file_last_modified[abs_file_path]
        ...
        threading.Thread(
            target=update_index_for_file, args=(self.directory, abs_file_path)
        ).start()
            ...
            try:
                ...
                if isinstance(content, bytes):
                    try:
                        ...
                        error_msg = (
                            f"Unable to decode document content: {doc_id}, error: {e!s}"
                        )
                        logger.warning(error_msg)
                        indexing_history_service.update_indexing_status(
                            doc, "failed", error_message=error_msg
                        )
...
def update_index_for_file(directory: Path, abs_file_path: Path) -> None:
    ...
    if not abs_file_path.is_file():
        logger.debug(
            "File does not exist or is not a file, skipping: %s", abs_file_path
        )
        return
...
async def index_remote_resource_async(resource: Resource) -> None:
    ...
    try:
        ...
        documents = [
            Document(text=markdown, doc_id=link) for link, markdown in link_md_pairs
        ]
        ...
        if all(results):
            ...
            resource_service.update_resource_indexing_status(
                resource.uri, "indexed", ""
            )
        else:
            failed_batches = len([r for r in results if not r])
            error_msg = (
                f"Some batches failed processing ({failed_batches}/{len(batches)})"
            )
            logger.error(error_msg)
            resource_service.update_resource_indexing_status(
                resource.uri, "indexed", error_msg
            )
...
async def add_resource(request: ResourceRequest, background_tasks: BackgroundTasks):  # noqa: D103, ANN201, C901
    ...
    if is_local_uri(request.uri):
        ...
        if not directory.is_dir():
            raise HTTPException(
                status_code=400, detail=f"{directory} is not a directory"
            )

        git_directory = directory / ".git"
        if not git_directory.exists() or not git_directory.is_dir():
            raise HTTPException(
                status_code=400, detail=f"{git_directory} ia not a git repository"
            )
    ...
    if resource:
        ...
        if exists_resource:
            raise HTTPException(
                status_code=400, detail="Resource with same name already exists"
            )
        # Add to database
        resource = Resource(
            id=None,
...
async def retrieve(request: RetrieveRequest):  # noqa: D103, ANN201, C901, PLR0915
    if is_local_uri(request.base_uri):
        ...
        if not directory.exists():
            raise HTTPException(
                status_code=404, detail=f"Directory not found: {request.base_uri}"
            )
    ...
    response_text = "".join(
        char for char in response_text if char.isprintable() or char in "\n\r\t"
    )
...
class IndexingStatusResponse(BaseModel):
    ...
    is_watched: bool = Field(
        ..., description="Whether the directory is currently being watched"
    )
    files: list[IndexingHistory] = Field(
        ..., description="List of files and their indexing status"
    )
    total_files: int = Field(
        ..., description="Total number of files processed in this directory"
    )
